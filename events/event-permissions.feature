...
Feature: Event publishing and subscribing
  ...
  Scenario: Clients subscribe to open events
    Given client A subscribes to an event named "open/some-event"
    When client B publishes an event named "open/some-event" with data "someData"
  ...
  Scenario: Allows only subscribing or publishing
    ...
    When client A publishes an event named "a-to-b/some-event" with data "someData"
  ...
  Scenario: Allows places in berlin
    When client A publishes an event named "place/berlin" with data { "address": {"city": "Berlin"} }
    Then client A received no errors
